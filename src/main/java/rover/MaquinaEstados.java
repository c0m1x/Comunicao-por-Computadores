...
public class MaquinaEstados {
    ...
    public static class RoverContext {
        // Identificadores
        public int idRover;
        public int idNave = 1;
        ...
        public RoverContext(int id, float posX, float posY) {
            this.idRover = id;
            this.posicaoX = posX;
            this.posicaoY = posY;
            this.bateria = 100.0f;
        ...
        public void atualizarEstadoOperacional(String estado) {
            lock.lock();
            try { this.estadoOperacional = estado; }
            finally { lock.unlock(); }
        }
            ...
            switch (e) {
                ...
                case ESTADO_EM_MISSAO:
                    ...
                    if (missaoConcluida()) {
                        ...
                        lock.lock();
                        try { eventoPendente = EventoRelevante.EVENTO_FIM_MISSAO; }
                        finally { lock.unlock(); }
        ...
        private void executarPassoMissao() {
            ...
            try {
                ...
                if (missaoAtual.duracaoMissaoSecs > 0) {
                    progressoMissao = (float)((decorrido / (double)missaoAtual.duracaoMissaoSecs) * 100.0);
                    if (progressoMissao > 100.0f) progressoMissao = 100.0f;
                }

                // checkpoints a cada 25%
                int checkpoint = (int)(progressoMissao / 25.0f);
                // simplificação: disparar evento quando ultrapassa o proximo checkpoint
                // (implementação leve — podes ajustar)
                if (checkpoint > 0) {
        ...
        private boolean missaoConcluida() {
            lock.lock();
            try { return progressoMissao >= 100.0f; }
            finally { lock.unlock(); }
        }
        ...
        public boolean deveEnviarTelemetria() {
            ...
            try {
                long agora = Instant.now().getEpochSecond();
                boolean deve = false;
                if ((agora - ultimoEnvioTelemetriaEpoch) >= INTERVALO_TELEMETRIA_BASE) deve = true;
        ...
        public void telemetriaEnviada() {
            lock.lock();
            try { ultimoEnvioTelemetriaEpoch = Instant.now().getEpochSecond(); }
            finally { lock.unlock(); }
        }

        public int getMissaoId() {
            lock.lock();
            try { return idMissaoAtual; }
            finally { lock.unlock(); }
        }

        public float getProgresso() {
            lock.lock();
            try { return progressoMissao; }
            finally { lock.unlock(); }
        }
